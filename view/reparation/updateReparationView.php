...
<section>
    <form action="index.php?entity=reparation&amp;action=updateReparation" method="post">

      <label for="idreparation">ID</label>
      <input type="text" class="form-control" name="idreparation" id="idreparation" value="<?php echo $_GET['id'] ?>" readonly /><br />
      ...
      <div style="padding: 30px;">
        ...
        <input type="text" class="form-control" name="description" id="description" value="<?php echo htmlspecialchars($reparation['description']) ?>" required maxlength="255" /><br />
        
        <label id="labelPrix" for="prix">Prix</label>
        <input type="number" class="form-control" name="prix" id="prix" min="0" max="999999" value="<?php echo $reparation['prix'] ?>" required /><br />
      ...
      ?>
        <option value="<?php echo $employe['idemploye']; ?>" 
                <?php if ($employe['idemploye'] == $reparation['technicien']) { echo 'selected'; } ?>
      ...
      {
      ?>
        <option value="<?php echo $voiture['idvoiture']; ?>" 
                <?php if ($voiture['idvoiture'] == $reparation['voiture']) { echo 'selected'; } ?>
                ><?php echo htmlspecialchars($voiture['immatriculation']) . ' (' . htmlspecialchars($voiture['marque']) . ' ' . htmlspecialchars($voiture['type']) . ' ' . $voiture['annee'] . ')'; ?></option>
      <?php
      }
